...
namespace DotNet.Tools.Tests
{
    class Testing
    {
        ...
        private void test_wts() 
        {
            WIN32.Win32_WTSSessions wt = new WIN32.Win32_WTSSessions("SVIGI-10");
            wt.Connect();
            WIN32.Win32_WTSSession[] sessions = wt.GetSessions();
            if (sessions != null) 
            {
                foreach (WIN32.Win32_WTSSession session in sessions) 
                {
                    Console.WriteLine(session.SessionID.ToString() + " - " + session.WorkstationName);
                }
            }

        }

        private void test_lan_server() 
        {
